...
public interface PurchaseDetailMapper {
	...
	class SqlProvider extends SQL{
		...
		public String insertPurchaseDetail(PurchaseOutDetail purchaseOutDetail){
			INSERT_INTO(schema+".FE8WMS11");
			VALUES("CTUKID", schema+".FE8WMS11_SEQ.NEXTVAL");
			//VALUES("ctmcu","#{ctmcu,jdbcType=NCHAR}");
			//VALUES("ctco","#{ctco,jdbcType=NCHAR}");
			VALUES("ctmcu","(select LSMCU from "+schema+".FE8WMS20 where trim(LSE8WLZXID) = #{ctmcu,jdbcType=NCHAR} and trim(LSE8HZBM) = #{ctco,jdbcType=NCHAR})");
			VALUES("ctco","(select LSCO from "+schema+".FE8WMS20 where trim(LSE8WLZXID) = #{ctmcu,jdbcType=NCHAR} and trim(LSE8HZBM) = #{ctco,jdbcType=NCHAR})");
			...
			VALUES("ctdcto","#{ctdcto,jdbcType=NCHAR}");
			return toString();
		}
	}
