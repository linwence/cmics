...
@Repository("fE84202BMapperC")
public interface FE84202BMapperC {
    @InsertProvider(
            type = FE84202BMapperC.SqlProvider.class,
    ...
    public static final class SqlProvider extends SQL {
        ...
        public String insertSelective(@Param("schema") String schema, @Param("record") Fe84202B record) {
            ...
            if(record.getAlukid() != null) {
                this.VALUES("ALKCOO", "(select SQKCOO From " + schema + ".Fe841001 where trim(squkidp) = #{record.alukid})");
            }

            if(record.getAlukid() != null) {
                this.VALUES("ALE8SPLX", "(select SQE8SPLX From " + schema + ".Fe841001 where trim(squkidp) = #{record.alukid})");
            }
