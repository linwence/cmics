...
public interface BalesMapper {
    ...
    class sqlProvider extends SQL {
        ...
        public String selectBalesList(String co) {
            String sql = "SELECT DISTINCT bslitm,LSE8HZBM,imlitm,spe8name,imsrtx,nvl(spe8jdsj,sysdate) spe8jdsj,imdsc2," +
                    "(SELECT DRDL01 FROM " + ctlSchema + ".f0005  WHERE drsy='E8' AND DRRT='17' AND trim(DRKY)=trim(sporig)) as sporig," +
                    "(SELECT DRDL01 FROM " + ctlSchema + ".f0005D WHERE drsy='00' AND DRRT='UM' AND trim(DRKY)=trim(IMUOM1)) as imuom1,imsld,case  when imstkt='P' THEN '启用'else '禁用'end imstkt,ccname,spe8tym,(case when spe8xlhgl='N' THEN '否' else '是'end) as spe8xlhgl,nvl(spe8yhzq,0) as spe8yhzq,imsrp3,imsrp5,(case when spe8zdyh='N' THEN '否' else '是' end ) as spe8zdyh,abalph," +
